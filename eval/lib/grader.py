...
WORLDVQA_TASKS = {"encyclopedic_vqa", "mmsearch", "worldvqa", "factualvqa", "webqa", "multimodalqa"}
...
def _normalize_text(s: str) -> str:
    s = re.sub(r"\b(a|an|the)\b", " ", s.lower().translate(
...
def _golds_for(task: str, od: dict):
    if task in EXACT_MATCH_TASKS:
        g = od.get("answers") or od.get("reference_list") or od.get("answer") or od.get("gt_answer")
...
def grade_exact_match(path: str) -> dict:
    ...
    for d in rows:
        ...
        if is_exact_match(strip_think(d.get("final_response")), golds):
            c += 1
    n = len(rows)
    return {"task": "exact_match", "file": path, "n": n, "correct": c,
            "incorrect": n - c, "unattempted": 0, "errors": 0,
            "score": c / n if n else 0.0}


async def grade_file(task: str, path: str, grader_model: str = DEFAULT_GRADER_MODEL,
    ...
    async def judge(i, d):
        ...
        if is_sqa:
            ...
            prompt = JUDGE_WORLDQA_PROMPT_EN.format(
                question=d.get("problem", ""), model_answer=answer, ground_truth_answer=gt)
        async with sem:
            try:
                r = await client.chat.completions.create(
                    model=grader_model, temperature=0,
                    max_tokens=GRADER_MAX_TOKENS, seed=GRADER_SEED,
                ...
                if is_sqa:
                    ...
                    labels[i] = {"A": "correct", "B": "incorrect", "C": "unattempted"}[letter]
                else:
    ...
    return {"task": task, "file": path, "n": n, "correct": c, "incorrect": inc,
            "unattempted": una, "errors": len(errs),
            "score": c / n if n else 0.0}
...
def main():
    ...
    args = ap.parse_args()
    res = asyncio.run(grade_file(args.task, args.jsonl, args.grader_model, args.concurrency))
    print(f"{Path(res['file']).name}: {res['correct']}/{res['n']} = {res['score']:.4f} "
